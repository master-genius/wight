...
const w = new function () {
  ...
  this.alert = function (info, options = {}) {
    ...
    astack.count++;
    if (options.context) {
      if (is_component) astack.compmap[aid] = options.context;
      else astack.pagemap[aid] = options.context;
    }
  ...
  this.cancelAlert = function (ctx) {
    ...
    delete this.alertStack.compmap[aid];
    delete this.alertStack.pagemap[aid];
  ...
  this.cancelPopup = this.cancelAlert;
  ...
  this.promptTargetMap = {
    promptmiddledom: null,
    promptdom: null,
    comp_promptmiddledom: null,
    comp_promptdom: null
  };
  ...
  this.prompt = function (info, options = {}) {
      ...
      let is_comp = !!options.is_comp;

      if (is_comp) {
        domname = 'comp_' + domname;
        closedom = 'comp_' + closedom;
      }

      if (!w[domname]) return false;

      w[domname].className = `w-prompt-box w-prompt-${wh} w-prompt-display`;
      if (is_comp) w[domname].className += (wh !== 'bottom') ? ' w-prompt-comp-top' : ' w-prompt-comp';
      let pcolor = '#424242';
      ...
      if (noclose) {
        ...
        if (wh !== 'bottom') {
          w[closedom].style.zIndex = is_comp ? 102 : 98;
          w[closedom].onclick = evt => {
            w.unprompt(false, is_comp);
          };
        } else {
          w[closedom].style.zIndex = is_comp ? 101 : 96;
          w[closedom].onclick = evt => {
            w.unprompt(true, is_comp);
          };
        }
      ...
      if (options.target) this.promptTargetMap[domname] = options.target;
  ...
  this.unprompt = async function (isbottom=true, is_comp=false) {
    ...
    if (this.promptTargetMap[domname]) {
      this.promptTargetMap[domname] = null;
    }
...
w._coreView = function (pagename, pg, data, k, nds) {
    if (data[k] === null) {
      return w._resetData(pagename, pg, nds);
    }
...
class Component extends HTMLElement {
  ...
  _coreView(data, k, nds) {
    if (data[k] === null) {
      return w._resetData(0, this, nds);
    }
  ...
  cancelPopup(aid='') {return this.cancelAlert(aid)}
  ...
  prompt(info, options={}) {
    ...
    options.target = this;
    options.is_comp = true;
    w.prompt(info, options);
  }

  unprompt(isbottom=true) {w.unprompt(isbottom, true);}
  unpromptMiddle(){w.unprompt(false, true);}
  unpromptTop(){w.unprompt(false, true);}
