...
class loaddoc {
    ...
    try {
      ...
      for (let i=0; i<dlist.length; i++) {
        ...
        if (dlist[i].isDirectory()) {
          ...
          this.loaddoc(pdir, pkg, dlist[i].name);
        }
      }
    } catch (err) {
  ...
  loaddoc (pdir, pkg, pname) {
    ...
    try {
      ...
      this.parsedoc(data, pkg, pname.substring(0, pname.length-3));
    } catch (err) {
      console.log(err);
    }
  }

  parsedoc (data, pkgdir, pname) {
    let di = {
      name : pname,
      description: '',
      keywords: '',
  ...
  markdata (mdata, pkgdir) {
    ...
    let htmldata = marked.parse(mdata, opts);
    htmldata = this.setImageStyle(htmldata);
    htmldata = htmldata.replace(/\<p\>/ig, '<p style="margin-top:0.5rem;margin-bottom:1.11rem;">');
    htmldata = htmldata.replace(/<\/pre>/ig, '</pre><br>');
    if (this.parsetn) {
      htmldata = htmldata.replace(/<\/p>/ig, '</p>');
      //opts = {gfm: true};
    }
  ...
  resetImgSrc (data, imgsrc, pkgdir) {
    let realsrc = '';
    if (imgsrc[0] === '.') realsrc = imgsrc.substring(1)

    if (imgsrc[0] == '/') {
      realsrc = imgsrc.substring(1);
    }

    realsrc = this.domain + this.routepath + '/' + pkgdir + '/' + realsrc;
  ...
  loadLecture (lecfile, leckey, lpath = '') {
      ...
      if (!lecdata.image) {
          ...
          lecdata.image = this.domain + this.routepath + lecdata.image;
      }
