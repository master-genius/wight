...
只是为了方便自己使用的一个前端小框架，前端热门的react、vue等，我一概不会，并且实在是不想去学这些东西。我主要是开发后端服务以及Linux运维和服务管理相关的工具。可是又离不开前端应用，于是开发了wight框架，后来几次更新升级，到2021年底，独立出wight2，做了一些不兼容修改，并且支持web components。

这个框架的参考了一点小程序的设计，只是从简单的功能和目录结构上，实际代码并未参考，但是整体结构还是区别很大。因为我讲过《微信小程序和公众号接口开发》的课程，所以为了在前端也能使用类似的形式做了一点尝试。
...
- 提供require在ext中用于导入扩展（require是异步操作，需要await require）。
...
- _components/ 各个组件放在此目录下。

- _static/ 静态资源目录，但是引入路径以static开头。

- _extends/ 扩展所在目录，使用exports导出扩展模块。
...
- components是要启用的组件，组件在_components目录下，\*表示启用所有组件，但是如果文件名以!开头则表示不启用，会忽略。如果指定部分组件，使用数组形式：
