...
获取共享数据，这会触发监听get操作的回调函数。这和获取w.share[key]的效果一致。

返回值是一个单独的数据或者是一个数组：

- 如果只有一个监听的函数，则这个函数的返回值如果不是undefined则返回，否则就是w.share代理的属性值。

- 如果有多个监听的函数，则返回一个数组，数组中包含所有监听函数的返回值。

### w.setShare(key, value)

设置共享数据，这会触发监听set操作的回调函数。这和设置w.share[key]的效果一致。
