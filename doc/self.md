...
在vue2.5的时候尝试过开发一些应用，因为一些问题而放弃，后来使用过一个比较小众的框架：mithril.js。也是因为种种问题都放弃了。再后来计划使用react，但是当时搭建环境竟然就要1000多个扩展的依赖，依赖越多，不稳定性越高，关键是这仅仅是一个前端环境，是一个不需要特别繁重的开发环境，但是需求变化却很剧烈的领域。
